...
function applyTranslatedFields_(f) {
  ...
  function renderQrList_(items) {
    ...
    rows.forEach(function (item) {
      ...
      // Payload is the canonical destination URL generated by CodeDesk "Mechanicals".
      // Source order:
      //  1) DestinationUrl column (when present)
      //  2) StateJson.fields (urlData + utm fields)  ✅ (restores prior behavior)
      //  3) legacy payload keys
      // IMPORTANT: do NOT fall back to Subtitle/subtitle; that is commonly a label like "CODEDESK — FLATTENED (PNG)".
      function buildPayloadFromStateJson_(it) {
        try {
          var raw = it && (it.StateJson || it.stateJson || it.state_json);
          if (!raw) return '';
          var obj = (typeof raw === 'string') ? JSON.parse(raw) : raw;
          var f = (obj && obj.fields) ? obj.fields : (obj || {});
          var base = String((f && f.urlData) || '').trim();
          if (!base) return '';

          var params = [];
          var us = String((f && f.utmSource) || '').trim();
          var um = String((f && f.utmMedium) || '').trim();
          var uc = String((f && f.utmCampaign) || '').trim();

          if (us) params.push('utm_source=' + encodeURIComponent(us));
          if (um) params.push('utm_medium=' + encodeURIComponent(um));
          if (uc) params.push('utm_campaign=' + encodeURIComponent(uc));

          if (!params.length) return base;

          // If base already has ?, append with &, otherwise start with ?
      ...
      var payloadText =
        // Canonical FileRoom column written by CodeDesk exports
        item.destination_url ||
        item.DestinationUrl ||

        // Reconstruct from FileRoom registry state (CodeDesk Mechanical fields)
        buildPayloadFromStateJson_(item) ||

        // Accept QR-ish payload keys (legacy)
        item.qrPayloadText ||
        item.PayloadText ||
        item.payloadText ||
        item.Payload ||
